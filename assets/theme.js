 ...
 BOLD.common.eventEmitter.emit("BOLD_COMMON_cart_loaded");
  ...
  theme.CartForm = (function() {
    ...
    function CartForm(form) {
      ...
      this.recommendations = form.querySelector(selectors.recommendations)
      this.itemCount = form.querySelectorAll(selectors.itemCount)
      this.submitBtn = form.querySelector(selectors.checkoutBtn);
  
      this.discounts = form.querySelector(selectors.discounts);
      this.savings = form.querySelector(selectors.savings);
      this.subtotal = form.querySelectorAll(selectors.subTotal);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function(html) {
        ...
        this.itemCount.forEach(element => {
          if(count == 0){
            element.innerHTML = ''
          }
          else if(count == 1){
            element.innerHTML = theme.translations.cart.general.items_with_count.one.replace('{{ count }}', count)
          }
          else {
            element.innerHTML = theme.translations.cart.general.items_with_count.other.replace('{{ count }}', count)
          }  
        })
        ...
        if(!!recommendations){
          fetch(theme.routes.cartPage + '?view=recommendations')
          .then(res => res.text())
          .then(text => {
            const div = document.createElement('div')
            div.innerHTML = text.trim()
            recommendations.innerHTML = div.querySelector('[data-recommendations]').innerHTML
          })
        }
        // Append item markup
        ...
        // Update subtotal
        this.subtotal = this.form.querySelectorAll(selectors.subTotal);
        this.subtotal.forEach(subt => {
          subt.innerHTML = theme.Currency.formatMoney(subtotal, theme.settings.moneyFormat);
        })
  ...
  theme.Drawers = (function() {
    ...
    Drawers.prototype = Object.assign({}, Drawers.prototype, {
      ...
      open: function(evt, returnFocusEl) {
        ...
        //Custom event when opening a drawer
  ...
  theme.Product = (function() {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      getImageSetName: function(string) {
        return string.toLowerCase().replace(/Ä/g, 'A').replace(/Ö/g, 'O').replace(/Ü/g, 'U').replace(/ä/g, 'a').replace(/ö/g, 'o').replace(/ü/g, 'u').replace(/[^a-z0-9]+/g, '-').replace(/-$/, '').replace(/^-/, '');
      },
  ...
  theme.Testimonials = (function() {
    ...
    Testimonials.prototype = Object.assign({}, Testimonials.prototype, {
      init: function() {
        ...
        defaults.pageDots = (this.slideshow.dataset.dots === "true");
        defaults.prevNextButtons = (this.slideshow.dataset.navArrows === "true");
...
document.addEventListener('variant:change', function(evt) {
  const featuredMediaId = evt.detail.variant.featured_media.id
  if(!!featuredMediaId){
    setTimeout(() => {
      document.querySelector(`[data-id="${featuredMediaId}"]`).click()
    },100)
  }
});